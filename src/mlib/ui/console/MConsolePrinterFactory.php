...
class MConsolePrinterFactory {
	...
	public static function setImplementation($class) {
		...
		if(!is_subclass_of($class, MConsolePrinter::class)){
			throw new \InvalidArgumentException(sprintf(
									'Class %s must extend %s',
									$class,
									MConsolePrinter::class
							));
		}
	 ...
	 * @param string $class
	 * @param array $args
	 * @return \mlib\ui\console\MConsolePrinter
	 * @throws \InvalidArgumentException
	 */
	...
	public static function createFromArray(array $array_config) {
		try{
			...
			throw new MConsolePrinterFactoryException("Problem with console printer config", 2, $e);
		}
	}
		...
		try{
			...
			foreach($additional_config->getConfigsNames() as $key){
				if(!in_array($key, $entries)){
					...
					throw new MConsolePrinterFactoryException("Cannot merge config from file $config_filename and additional config object : some key already exist in the file");
				}
			}
			$class = static::$implClass ?: MConsolePrinter::class;
			return self::createInstance($class, $config);
			...
			throw new MConsolePrinterFactoryException("Problem with console printer config file", 3, $e);
		}
	}
	...
	public static function createFromFileAndArray(string $config_filename, array $additional_array_config) {
		try{
			...
			throw new MConsolePrinterFactoryException("Problem with console printer config file", 4, $e);
		}
	}
}
?>
