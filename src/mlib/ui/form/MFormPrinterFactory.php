...
class MFormPrinterFactory {
	...
	public static function setImplementation($class) {
		...
		if(!is_subclass_of($class, MFormPrinter::class)){
	 ...
	 * @param string $class
	 * @param array $args
	 * @return \mlib\ui\form\MFormPrinter
	 * @throws \InvalidArgumentException
	 */
	 ...
	 * @return \mlib\ui\form\MFormPrinter
	 * @throws MFormPrinterFactoryException
	 */
	public static function createFromFile(string $config_filename, array $custom_options = null) {
		try{
			...
			$class = static::$implClass ?: MFormPrinter::class;
			return self::createInstance($class, $config, $custom_options);
		}
		catch(\mlib\utils\config\MConfigException $e){
			throw new MFormPrinterFactoryException("Problem with form printer config file", 1, $e);
		}
	}
	...
	public static function createFromArray(array $array_config, array $custom_options = null) {
		try{
			...
			throw new MFormPrinterFactoryException("Problem with form printer config", 2, $e);
		}
	}
		...
		try{
			...
			foreach($additional_config->getConfigsNames() as $key){
				if(!in_array($key, $entries)){
					...
					throw new MFormPrinterFactoryException("Cannot merge config from file $config_filename and additional config object : some key already exist in the file");
				}
			}
			$class = static::$implClass ?: MFormPrinter::class;
			return self::createInstance($class, $config, $custom_options);
			...
			throw new MFormPrinterFactoryException("Problem with form printer config file", 3, $e);
		}
	}
	...
	public static function createFromFileAndArray(string $config_filename, array $additional_array_config, array $custom_options = null) {
		try{
			...
			throw new MFormPrinterFactoryException("Problem with form printer config file", 4, $e);
		}
	}
}
?>
